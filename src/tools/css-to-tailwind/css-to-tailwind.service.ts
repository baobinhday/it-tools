...
const TRANSFORM_PATTERNS: TransformPattern[] = [
  ...
  // This is the new pattern for single-value translate. It should produce both translate-x and translate-y.
  { regex: /^translate\(([^)]+)\)$/, format: m => `translate-x-[${m[1]}] translate-y-[${m[1]}]` },
  { regex: /^translate\(([^,]+),\s*([^)]+)\)$/, format: m => `translate-x-[${m[1]}] translate-y-[${m[2]}]` },
  { regex: /^rotate\(([^)]+)\)$/, format: m => `rotate-[${m[1]}]` },
  { regex: /^skewX\(([^)]+)\)$/, format: m => `skew-x-[${m[1]}]` },
  { regex: /^skewY\(([^)]+)\)$/, format: m => `skew-y-[${m[1]}]` },
  { regex: /^skew\(([^,]+),\s*([^)]+)\)$/, format: m => `skew-x-[${m[1]}] skew-y-[${m[2]}]` },
  // This is the new pattern for single-value skew. It should produce skew-x.
  { regex: /^skew\(([^)]+)\)$/, format: m => `skew-x-[${m[1]}]` },
];
...
function mapSpacing(prefix: string, value: string, remInPx: number = 16): string {
  ...
  if (value.endsWith('px')) {
    ...
    if (px === 0) {
      return `${prefix}-0`; // Keep the fix from the previous task
    }
    canonical = `${px / remInPx}rem`;
  }

  const token = SPACING_TOKEN[canonical];
  if (!token) {
    return `${prefix}-[${value}]`;
  }
  if (token === '0') {
...
type CssToTwFn = (value: string, remInPx: number) => string | null;

const cssToTailwindMap: Record<string, CssToTwFn> = {
  ...
  'padding': (value: string, remInPx: number) => {
  ...
  'padding-left': (v: string, remInPx: number) => mapSpacing('pl', v, remInPx),
  'padding-right': (v: string, remInPx: number) => mapSpacing('pr', v, remInPx),
  'padding-top': (v: string, remInPx: number) => mapSpacing('pt', v, remInPx),
  'padding-bottom': (v: string, remInPx: number) => mapSpacing('pb', v, remInPx),
  'margin': (value: string, remInPx: number) => {
    const parts = value.trim().split(/\s+/);
    const { top, right, bottom, left } = expandBoxShorthand(parts);
  ...
  'margin-left': (v: string, remInPx: number) => mapSpacing('ml', v, remInPx),
  'margin-right': (v: string, remInPx: number) => mapSpacing('mr', v, remInPx),
  'margin-top': (v: string, remInPx: number) => mapSpacing('mt', v, remInPx),
  'margin-bottom': (v: string, remInPx: number) => mapSpacing('mb', v, remInPx),
  ...
  'transform': (value: string) => {
    ...
    return simple ?? `[transform:${value.replace(/\s+/g, '_')}]`;
  },
  'cursor': (value: string) => {
                ...
                if (rule.block && rule.block.children) {
                  rule.block.children.forEach((childNode: CssNode) => {
                    if (childNode.type === 'Declaration') {
                      ...
                      const fn = cssToTailwindMap[property];
                      if (fn) {
                        const tailwindClass = fn(processedValue, remInPx);
                        if (tailwindClass) {
